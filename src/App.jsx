...
// ═══════════════════════════════════════════════════════════
// LOGIN ADMIN — entrada para a "oficina de bastidores"
// ═══════════════════════════════════════════════════════════
...
const PainelAdmin = ({mercados,setMercados,parts,setParts,emola,setEmola,mpesa,setMpesa,cfg,setCfg,sair}) => {
  ...
  const publicar=()=>{
    ...
    const novo={id:Date.now(),cat:form.cat,q:form.q,emoji:form.emoji||CAT_I[form.cat]||"❓",prazo:form.prazo,sim:0,nao:0,destaque:form.destaque,status:"aberto",criado:hoje(),vol:[0],met:form.met,fontes:form.fontes};
  ...
  const resolver=(id,venc)=>{ setMercados(p=>p.map(m=>m.id===id?{...m,status:"resolvido",venc}:m)); setResolverM(null); notif(`✅ Resolvido — ${venc.toUpperCase()} ganhou!`); };
  const confirmarP=id=>{setParts(p=>p.map(a=>a.id===id?{...a,status:"confirmado"}:a));notif("✅ Confirmada!");};
  const rejeitarP =id=>{setParts(p=>p.map(a=>a.id===id?{...a,status:"rejeitado"}:a)); notif("🚫 Rejeitada");};
  const apagarP   =id=>{setParts(p=>p.filter(a=>a.id!==id)); notif("🗑️ Removida");};
  const apagarM   =id=>{setMercados(p=>p.filter(m=>m.id!==id)); notif("🗑️ Removida");};
  const alternarD =id=>{setMercados(p=>p.map(m=>m.id===id?{...m,destaque:!m.destaque}:m));};
...
// ═══════════════════════════════════════════════════════════
function ContadorPrazo({prazo}) {
  const calcRest = () => {
    const fim = new Date(prazo.split(" ").reverse().map((p,i)=>{
      const meses={"Jan":"01","Fev":"02","Mar":"03","Abr":"04","Mai":"05","Jun":"06","Jul":"07","Ago":"08","Set":"09","Out":"10","Nov":"11","Dez":"12"};
      if(i===0) return p;
      if(i===1) return meses[p]||"01";
      return p;
    }).join("-"));
    ...
    if(meses>3) return {txt:`${meses} meses`,cor:P.textoS};
    if(dias>7)  return {txt:`${dias} dias`,cor:"#FFB020"};
    return {txt:`${dias} dias`,cor:P.rubi};
  };
  const r = calcRest();
  if(!r) return <span style={{fontSize:10,color:P.rubi,fontFamily:F.body}}>⏰ Encerrado</span>;
  return <span style={{fontSize:10,color:r.cor,fontFamily:F.body}}>⏰ {r.txt} restantes</span>;
}
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    storageSet("mz_mercados",mercados);
    storageSet("mz_parts",parts);
    storageSet("mz_cfg",{emola,mpesa,...cfg});
  },[mercados,parts,emola,mpesa,cfg,carregando]);
